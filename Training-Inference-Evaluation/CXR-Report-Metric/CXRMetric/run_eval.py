...
NORMALIZER_PATH = "/home/avnish/CXR-ReDonE/CXR-Report-Metric/CXRMetric/normalizer.pkl"
COMPOSITE_METRIC_V0_PATH = "/home/avnish/CXR-ReDonE/CXR-Report-Metric/CXRMetric/composite_metric_model.pkl"
COMPOSITE_METRIC_V1_PATH = "/home/avnish/CXR-ReDonE/CXR-Report-Metric/CXRMetric/radcliq-v1.pkl"

REPORT_COL_NAME = "findings"
STUDY_ID_COL_NAME = "uid"
COLS = ["bertscore", "semb_score", "bleu_score"]

cache_path = "/home/avnish/CXR-ReDonE/CXR-Report-Metric/CXRMetric/cache/"
...
def add_bleu_col(gt_df, pred_df):
    ...
    for i, row in gt_df.iterrows():
        ...
        if len(pred_row) == 1:
            ...
            score = sentence_bleu([gt_report], predicted_report, weights=(0.5, 0.5), smoothing_function=smoothing_function)
...
def calc_metric(gt_csv, pred_csv, out_csv, use_idf): # TODO: support single metrics at a time
    ...
    gt_study_ids = (gt[STUDY_ID_COL_NAME])
    pred_study_ids = (pred[STUDY_ID_COL_NAME])
    # shared_study_ids = gt_study_ids.intersection(pred_study_ids)
    # print(f"Number of shared study IDs: {len(shared_study_ids)}")
    gt = gt.loc[gt[STUDY_ID_COL_NAME].isin(gt_study_ids)].reset_index()
    pred = pred.loc[pred[STUDY_ID_COL_NAME].isin(pred_study_ids)].reset_index()
    ...
    os.system(f"python /home/avnish/CXR-ReDonE/CXR-Report-Metric/CXRMetric/CheXbert/src/encode.py -c {CHEXBERT_PATH} -d {cache_pred_csv} -o {pred_embed_path}")
    os.system(f"python /home/avnish/CXR-ReDonE/CXR-Report-Metric/CXRMetric/CheXbert/src/encode.py -c {CHEXBERT_PATH} -d {cache_gt_csv} -o {gt_embed_path}")
